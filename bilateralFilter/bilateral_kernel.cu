...
d_bilateral_filter(uint *od, int w, int h,
                   float e_d,  int r, cudaTextureObject_t rgbaTex)
{
    ...
    od[y * w + x] = rgbaFloatToInt(center + float4{0.f, 0.f, 1.0f, 0.f});// rgbaFloatToInt(t / sum);
}
...
//texture<uchar4, 2, cudaReadModeNormalizedFloat> rgbaTex;
    ...
    checkCudaErrors(cudaCreateTextureObject(&rgbaTexdImage, &texRes, &texDescr, NULL));
